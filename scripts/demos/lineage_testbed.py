"""Lineage testbed — non-trivial pipeline that exercises the lineage UX features.
...
running to validate the features visually.
...
_PIPELINE_NAME = "Lineage testbed — plant cost analysis"
_RNG_SEED = 42
...
def _build_pipeline_doc(
    ...
    nodes: list[dict] = [
        ...
        # group_aggregate by plant — final summary
        {
            ...
            # Freshness policy on the terminal node — drives the freshness halo.
            "ui": {
                "x": 2280, "y": 80,
    ...
    return {
        ...
        "name": _PIPELINE_NAME,
        "description": (
            "Lineage UX testbed — exercises run-state, freshness, "
        ...
        "nodes": nodes,
        "outputs": [],
        # Group the cost-calculation steps so the user
...
def main() -> None:
    ...
    print("→ running pipeline (so the canvas has run-state to render)...")
    ...
    print(f"   http://localhost:3100/pipelines/{pid}")
    print()
    print("What to look for:")
    print("  • Run-state — every step has a green strip; cost_summary chips show row count + time ago")
    print("  • Freshness — cost_summary has a green halo (turns amber after 90m, red after 2h)")
    print("  • Column trace — hover `total_cost` column header → canvas dims off-lineage nodes")
    print("  • Groups — dashed 'Cost calculation' group around the 4 derive steps")
