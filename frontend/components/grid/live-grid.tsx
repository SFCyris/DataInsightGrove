    ...
    <div className="flex flex-col h-full min-h-0 relative">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {(() => {
                          ...
                          // breaks if they change this column's expression
                          // / type / name. Hidden when count = 0 to keep
                          // pure-passthrough columns visually quiet.
                          const n = downstreamImpact?.[c.name];
                          ...
                          return (
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                onColumnAction({ kind: "column_dna", column: c.name });
                              }}
                              title={`Used by ${n} downstream node${n === 1 ? "" : "s"} — click for full impact map`}
                              aria-label={`${n} downstream consumer${n === 1 ? "" : "s"} — opens Column DNA`}
                              className="ml-1 inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 border border-emerald-300/60 hover:bg-emerald-200 dark:hover:bg-emerald-900/60 transition-colors cursor-pointer"
                            >
                              → {n}
                            </button>
                          );
      ...
      <ProfileDrawer
        ...
        onRangeFilter={(column, low, high) => {
          ...
          // `column_action` flow: a derived predicate step on the grid.
          // We piggyback on `cell_quick_filter` for value-eq filters
          // and emit a small synthetic "range_filter" intent for the
          // numeric histogram bars. The page handler chooses how to
          // interpret it.
          onColumnAction({ kind: "range_filter", column, low, high } as ColumnAction);
        }}
      />
    </div>
  );
