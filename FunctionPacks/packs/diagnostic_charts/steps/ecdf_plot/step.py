...
class EcdfPlotStep(Step):
    def execute_polars(
        ...
        if pdf.empty:
            raise ValueError("ecdf_plot: no non-null rows")

        fig, ax = plt.subplots(figsize=(8, 5), dpi=144)
        sns.ecdfplot(data=pdf, x=value, hue=group, ax=ax)
        ax.set_title(title)
        ...
        return PolarsResult(
            ...
            artifacts=[{
                ...
                "chart": "ecdf", "title": title, "rows_plotted": pdf.shape[0],
            }],
        )


step = EcdfPlotStep(json.loads((Path(__file__).parent / "manifest.json").read_text()))
