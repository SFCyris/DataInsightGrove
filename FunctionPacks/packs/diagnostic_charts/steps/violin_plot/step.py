...
class ViolinPlotStep(Step):
    def execute_polars(
        ...
        if group:
            sns.violinplot(data=pdf, x=group, y=value, ax=ax, inner="box")
        else:
            sns.violinplot(data=pdf, y=value, ax=ax, inner="box")
        ax.set_title(title)
        fig.tight_layout()
        ...
        return PolarsResult(
            ...
            artifacts=[{
                ...
                "chart": "violin", "title": title, "rows_plotted": pdf.shape[0],
            }],
        )


step = ViolinPlotStep(json.loads((Path(__file__).parent / "manifest.json").read_text()))
