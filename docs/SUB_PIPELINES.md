...
This is the closest equivalent in DIG to Alteryx's "macros" or
Trifacta's "recipes-as-steps", built without their proprietary
mechanics. Three design choices worth knowing up-front:
...
> ⚠️ MVP: pipelines with multiple datasets can't be published yet.
> Reduce to one input dataset, or wait for multi-input support.

Multi-input composition is a future iteration; the doc format already
allows multiple aliases under `exposedInputs[]` so the schema won't
change when we lift the restriction.
...
2. **At run-start** (POST /pipelines/{id}/runs). Same check, idempotent.
   ...
   write, future migration) can't cause a runaway expansion.
...
- **Single input / single output.** Lifted in a future iteration.
...
- **Iterate-over-group ("batch macro") not in v1.** Composition layer
  exists; iteration over distinct values of a column is a separate
  feature on top.
