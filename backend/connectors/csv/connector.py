...
def _path(uri: str) -> Path:
    # ``assert_local_path_safe`` rejects URIs that resolve outside DIG's
    # data dir or the bundled samples folder. Without this guard the CSV
    # connector accepted ``file:///etc/passwd`` and read its contents
    # into a dataset (CVE-class arbitrary local file read — pen-tester
    # finding round 2). Bypassable via ``DIG_LOCAL_FILE_ALLOW_ABSOLUTE=1``
