...
class JoinStep(Step):
    ...
    def column_dependencies(self, input_schemas, params):
        # Mirror infer_schema's name-mapping logic so the lineage map's
        # output column names match exactly what infer_schema declares —
        # otherwise the lineage tracer can't connect downstream
        # references back through the join.
        left = input_schemas.get("left", {}) or {}
        right = input_schemas.get("right", {}) or {}
        kind = (params.get("kind") or params.get("how") or "inner").lower()
        if kind not in _VALID_KINDS:
        ...
        def emit(
            ...
            if prov_id in excluded:
                return
            name = renames.get(prov_id, default_name)
            out[name] = ColumnLineage(
                sources=sources,
                transform=transform,
                expression=None,
                is_passthrough=(transform == "passthrough"),
            )
        ...
        if kind == "cross":
            for k in left:
                emit(f"L:{k}", k, [ColumnRef(port="left", column=k)], "passthrough", out)
            for k in right:
                default = f"{k}_1" if k in left else k
                emit(f"R:{k}", default, [ColumnRef(port="right", column=k)], "passthrough", out)
            return out
        ...
        if kind == "anti_right":
            for k in right:
                emit(f"R:{k}", k, [ColumnRef(port="right", column=k)], "passthrough", out)
            return out
        ...
        for col in left:
            if col in right_set and col not in left_keys_eq:
                if rule == "keep_left":
                    emit(f"L:{col}", col, [ColumnRef(port="left", column=col)], "passthrough", out)
                elif rule == "keep_right":
                    pass
                elif rule == "coalesce":
                    emit(
                        ...
                        col,
                        [
                            ColumnRef(port="left", column=col),
                            ColumnRef(port="right", column=col),
                        ],
                        "coalesce(left, right)",
                        out,
                    )
        ...
        for col in right:
            ...
            if col in left_set:
                if rule == "keep_right":
                    emit(f"R:{col}", col, [ColumnRef(port="right", column=col)], "passthrough", out)
                elif rule == "keep_both":
                    emit(
                        f"R:{col}",
                        col + suffixes[1],
                ...
                # keep_left / coalesce: skip — already emitted above
            else:
                emit(f"R:{col}", col, [ColumnRef(port="right", column=col)], "passthrough", out)
